...
if True:
    ...
    for file in docFiles:
        ...
        ##    frame = GenFrame(width, height)
            ...
            for i in range(20):
                for j in range(20):
                    corner = SELarea2pic(i,j,width,height)
                    corner.append(1)
                    corner = np.array(corner)
                    c = np.matmul(H, corner)#cv2.perspectiveTransform(coord, H[0])
        ##            c = np.absolute(c)
        ##            c[:-1] = rot90DEG_left(c[:-1])
        ##            c[:-1] = np.matmul(np.array([[1, 0],[0, -1]]), c[:-1])
                    luLocP.append(c)
            print('\n\n\n', luLocP, '\n\n\n')

            AddCorners(luLocP, pano.shape[:2])
