...
##DF_left   = pd.read_csv(path1 + List[num[0]])
##DF_middle = pd.read_csv(path1 + List[num[1]])
##DF_right  = pd.read_csv(path1 + List[num[2]])
        
##csvPath = List[num]
##
####    left  = input('Rows to delete on left side:')
####    right = input('Rows to delete on right side:')
##
